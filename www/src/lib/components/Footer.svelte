<script lang="ts">
	...
	function readCache(): { count: number; fetchedAt: number } | null {
		try {
	...
	if (navigator.onLine && (!cached || Date.now() - cached.fetchedAt > CACHE_TTL_MS)) {
		refreshStarCount();
	}
</script>

<footer class="flex justify-end px-4 py-6">
	<a
		href={PROFILE_URL}
		target="_blank"
